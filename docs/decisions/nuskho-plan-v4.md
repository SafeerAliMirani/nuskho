...
Version 4. An advisor drew the plan, two independent reviewers attacked it — the
second with the shipped code open beside the document, which is how the
contradictions in "What the code says" were found — and then Safeer answered the
questions it had been holding open. Where anyone disagreed, the disagreement is
shown rather than smoothed over.

**What is now decided.** The product is a **downloaded application**, not a web
page; the link is a demo. Inside the clinic, devices talk over the **clinic's own
network**. Outside it, the doctor's own records are pushed to his own devices
**read-only**. Two-way multi-writer sync is not built at all. The working day
opens and closes on the machine; reporting anyone's hours to a hospital owner is
a separate product for another year.

**What is still open:** the price, who owns a hospital's records when a doctor
leaves, and one doctor working at two clinics.

Five sections are Safeer's corrections rather than the reviewers':
...
- **Cloud accounts.** The plan had argued against putting data on a server and
  had accidentally argued against putting *accounts* there too.
- **Three devices.** A doctor on a laptop, a phone and a tablet cannot have
  local-only history. This is what answered question 3.
- **The thirty-day session.** It authorises the *device*, never the person —
  otherwise the cleaner who has watched the doctor type it owns the machine at
  night.
- **The working day.** Not a report to management. The clinic knowing its own
  hours, which is a far smaller and far safer thing.

Almost nothing here is built. That is still the point of the document — but four
fixes that could not wait are now in the code, and they are marked where they
appear.
